...
class _MapPageState extends State<MapPage> {
  ...
  void initState() {
    ...
    beaches = [
      ...
      ...widget.allBeaches.map((beach) => Beach(
        ...
        location: beach['location'] ?? 'Unknown Location',
        coordinates: List<double>.from(beach['coordinates'] ?? [0.0, 0.0]),
      )),
    ];
    _fetchTemperatures();
  }
  ...
  Future<double> _getTemperature(double lat, double lon) async {
    ...
    if (response.statusCode == 200) {
      ...
      final temperatureValue = data['current_weather']['temperature'];
      if (temperatureValue == null) {
        throw Exception('Temperature data is null');
      }
